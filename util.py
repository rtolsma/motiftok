...
def one_hot_seq(seq):
    return array([[1 if i == DEFAULT_LETTER_TO_INDEX[nuc] else 0 for i in range(4)] for nuc in seq])

def background_prob_seq(seq_len, bg=None):
    if not bg:
        bg = DEFAULT_BACKGROUND_FREQ
    return array([list(bg.values()) for _ in range(seq_len)])
